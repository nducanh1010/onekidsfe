<template>
  ...
  >
    ...
    >
      <el-row :gutter="20">
        <el-col :span="16">
          <el-form-item label="Tên khoản" prop="fnPackage.name">
            <el-input
              disabled
              v-model="dataInput.fnPackage.name"
              placeholder="Nhập tên khoản"
            ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item label="Loại" prop="fnPackage.category">
            <el-select
              disabled
              v-model="dataInput.fnPackage.category"
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="Đơn vị tính" prop="fnPackage.unit">
            <el-input
              disabled
              v-model="dataInput.fnPackage.unit"
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item label="Kiểu đóng" prop="type">
            <span v-if="dataInput.fnPackage.type == 'single'">Một lần</span>
            <span v-else-if="dataInput.fnPackage.type == 'multiple'">Nhiều lần</span>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row v-if="dataInput.fnPackage.type == 'multiple'" :gutter="20">
        <el-col :span="24">
      ...
      <el-row v-if="!!dataInput.fnPackage.type" :gutter="20">
        <el-col :span="8">
        ...
        <el-col v-if="!!dataInput.fnPackage.type && dataInput.expired" :span="12">
          <el-form-item
            v-if="dataInput.fnPackage.type == 'single'"
            label="Hạn thu"
            prop="endDateExpired"
          >
            <el-date-picker
          ...
          <el-form-item
            v-if="dataInput.fnPackage.type == 'multiple'"
            label="Hạn tháng thu"
            prop="monthNumber"
          >
      ...
      <el-row
        v-if="dataInput.fnPackage.type == 'multiple' && dataInput.expired"
        :gutter="20"
      >
        <el-col :span="8">
      ...
      <el-row :gutter="20" v-if="dataInput.fnPackage.type == 'multiple'">
        <el-col :span="8">
          <el-form-item label="Áp dụng điểm danh" prop="attendance">
            <span v-if="dataInput.fnPackage.attendance">Có</span>
            <span v-else>Không</span>
          </el-form-item>
        </el-col>
        <el-col :span="8" v-if="dataInput.fnPackage.attendance">
          <el-form-item label="Kiểu trả" prop="attendancePaid">
            <span v-if="dataInput.fnPackage.attendancePaid == 'before'">Trả trước</span>
            <span v-else-if="dataInput.fnPackage.attendancePaid == 'after'">Trả sau</span>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row
        :gutter="20"
        v-if="dataInput.fnPackage.type == 'multiple' && dataInput.fnPackage.attendance"
      >
        <span v-if="dataInput.fnPackage.attendancePaid == 'before'">
          <el-col :span="8">
            <el-form-item label="Loại điểm danh" prop="attendanceType">
              <el-select
                disabled
                v-model="dataInput.fnPackage.attendanceType"
          ...
          <el-col :span="8" v-if="dataInput.fnPackage.attendanceType != 'pickupLate'">
            <el-form-item label="Chi tiết" prop="attendanceDetail">
              <el-select
                disabled
                v-model="dataInput.fnPackage.attendanceDetail"
        ...
        <span v-else-if="dataInput.fnPackage.attendancePaid == 'after'">
          <el-col :span="8">
            <el-form-item label="Loại điểm danh" prop="attendanceType">
              <el-select
                disabled
...
export default {
  ...
  computed: {
    getAttendanceDetailList() {
      let detailList = [];
      let dataInputComputed = this.dataInput.fnPackage;
  ...
  methods: {
    ...
    checkBeforeSubmitMonth() {
      ...
      if (dataInputCheck.fnPackage.type == "multiple") {
        if (
          !dataInputCheck.t1 &&
          !dataInputCheck.t2 &&
          !dataInputCheck.t3 &&
    ...
    submitForm() {
      this.$refs["dataInput"].validate((valid) => {
        if (valid) {
          ...
          FnFeesService.updatePackageDefault(this.dataInput)
            .then((resp) => {
              this.$message({
    ...
    getDataInitial(id) {
      FnFeesService.getPackageDefaultById(id)
        .then((resp) => {
