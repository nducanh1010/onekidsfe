<template>
  <el-dialog
    :title="'Khoản mặc định của lớp ' + className + ': Bổ sung cho tháng hiện tại'"
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      let idPackageList = this.multipleSelection.map((x) => x.id);
      if (idPackageList.length == 0) {
        this.$message({
      ...
      FnFeesService.addPackageKidsFromPackageDefaultMany(this.idKidList, idPackageList)
        .then((resp) => {
          this.$message({
    ...
    getAddDataInitial(idClass, className, idKidList) {
      this.idKidList = idKidList;
      this.className = className;
      FnFeesService.searchPackageKidsDefaultInClass(idClass)
        .then((resp) => {
