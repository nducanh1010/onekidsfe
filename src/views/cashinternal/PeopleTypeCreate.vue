  ...
  >
    ...
    >
      <el-form-item label="Tên đối tượng" prop="name">
        <el-input v-model="dataInput.name" placeholder="Nhập tên đối tượng"></el-input>
      </el-form-item>
      ...
      <div class="gender" style="float: right; margin-top: -65px">
        <el-form-item label="Số CMND" prop="indentify">
          <el-input
...
export default {
  components: { CashInternalPayCreate },
  props: {
    dialogVisible: null,
  },
  data() {
    ...
    submitForm() {
      this.$refs["dataInput"].validate((valid) => {
        if (valid) {
          ...
          CashInternalService.createPeopleType(this.dataInput)
            .then((resp) => {
              this.$message({
