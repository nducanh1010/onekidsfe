  ...
  <div class="content">
    ...
    <div class="search">
      ...
      >
        ...
        <el-button
          v-if="checkPermission(['cashInternal_in_create'])"
          class="button-over"
          type="success"
        ...
        <el-dropdown @command="handleCommand">
          ...
          <el-dropdown-menu>
            <el-dropdown-item
              v-if="checkPermission(['cashInternal_in_approved'])"
              command="approve"
            ...
            >
            <el-dropdown-item command="cancel">Hủy phiếu thu</el-dropdown-item>
    ...
    <div class="table-content row-data">
      ...
      >
        ...
        <el-table-column prop="price" align="right" label="Số tiền" width="120">
          <template slot-scope="scope">
            <span v-if="scope.row.money == 0">{{ 0 }}&nbsp;₫</span>
            <span v-else>{{ scope.row.money | formatCurrency }}</span>
          </template>
        </el-table-column>
        <el-table-column label="Ngày thu" width="120" align="center">
          <template slot-scope="scope">
        ...
        >
          <template slot-scope="scope">
            ...
            <span
              style="margin-left: 10px"
              v-if="checkPermission(['cashInternal_in_payment'])"
            >
              <el-button
                v-if="
                  scope.row.approved &&
    ...
    <CashInternalCollectMoneyCreate
...
import CashInternalCollectMoneyCreate from "./CashInternalCollectMoneyCreate.vue";
import CashInternalPayUpdate from "./CashInternalPayUpdate.vue";
import CashInternalDetail from "./CashInternalDetail.vue";
import StatisticalInDialog from "./StatisticalInDialog.vue";
export default {
  props: {
    dialogVisible: null,
    editGrade: null,
  ...
  methods: {
    ...
    createDialogMethod() {
      this.$refs.CashInternalCollectMoneyCreate.getDataInternalBeforeSelect();
    ...
    viewStatistical() {
      this.$refs.StatisticalInDialog.getDataInitial();
      this.showStatistical = true;
    },
    handlePayment(row) {
      row.loaddingButtonRow = true;
      CashInternalService.updatePaymentIn(row.id)
        .then((resp) => {
    ...
    checkSelectRowList(rowList, text) {
      if (rowList.length == 0) {
        this.$message({
          message: "Không có phiếu thu nào được chọn ở trạng thái " + text,
          type: "error",
        });
        throw null;
      }
    ...
    approvedOneMethod(row, status) {
      CashInternalService.updateApproveIn(row.id, status)
        .then((resp) => {
          this.$message({
            message: resp.data.message,
            type: "success",
    ...
    cancelManyMethod() {
      ...
      this.$confirm(
        ...
        CashInternalService.cancelManyIn(dataInput)
          .then((resp) => {
            this.$message({
              message: resp.data.message,
              type: "success",
    ...
    approveManyMethod(status, text) {
      ...
      this.$confirm(
        ...
        CashInternalService.updateApproveManyIn(dataInput)
          .then((resp) => {
            this.$message({
              message: resp.data.message,
              type: "success",
    ...
    searchByProperties() {
      ...
      this.textTable = "";
      CashInternalService.searchcollectcash(
        this.pageNumber,
        this.maxPageItem,
        this.dateStartEnd,
