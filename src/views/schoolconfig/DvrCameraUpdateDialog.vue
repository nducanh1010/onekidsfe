...
export default {
  ...
  methods: {
    ...
    submitForm(fromData) {
      let dataSubmit = this.dvrCameraData;
      if (this.dvrCameraData.dvrType != "Khác") {
        dataSubmit.linkDvr = this.changeData;
      }
      this.$refs[fromData].validate((valid) => {
        if (valid) {
          this.loadingButton = true;
          SchoolConfigService.updateDvrCamera(dataSubmit)
            .then((response) => {
              let data = response.data.data;
              console.log(data);
              this.$message({
                message: "Cập nhật đầu ghi thành công",
                type: "success",
              });
            ...
            .catch((err) => {
              this.$message({
                message: "Cập nhật đầu ghi thất bại",
                type: "error",
              });
              console.log(err);
            })
    ...
    getDvrcameraUpdateInitial(row) {
      this.dvrCameraData = row;
    },
  },
};
</script>
