...
export default {
  ...
  methods: {
     ...
     */
    submitForm(fromData) {
      let kidList = this.evaluateMonthDataList;
      if (kidList.length == 0) {
        this.$message({
      ...
      this.$confirm(
        ...
        EvaluateService.saveEvaluateMonthCommon(formDatas)
          .then((resp) => {
            let checkUpdate = resp.data.data;
            console.log(checkUpdate);
            this.$message({
              message: "Lưu nhận xét tháng cho nhiều học sinh thành công",
              type: "success",
            });
            setTimeout(() => {
          ...
          .catch((err) => {
            console.log(err);
            this.$message({
              message: "Lưu nhận xét tháng cho nhiều học sinh thất bại",
              type: "error",
            });
          })
          .finally(() => {
    ...
    getEvaluateMonthKidsInitial(data) {
      this.evaluateMonthDataList = data;
    },
  },
};
</script>
