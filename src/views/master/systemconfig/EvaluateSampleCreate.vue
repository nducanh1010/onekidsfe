<template>
  ...
  >
    <el-form
      label-width="120px"
      :model="newEvaluateSample"
      label-position="left"
      :rules="rules"
      ref="newEvaluateSample"
    >
      <el-form-item label="Loại nhận xét" prop="evaluateType">
        <el-select
          style="width:100%"
          v-model="newEvaluateSample.evaluateType"
      ...
      <el-form-item label="Nội dung" prop="evaluateContent">
        <el-input
          :rows="15"
          type="textarea"
          v-model="newEvaluateSample.evaluateContent"
          placeholder="Nhập nội dung"
        ></el-input>
      </el-form-item>
    </el-form>
    <span slot="footer" class="dialog-footer">
      <el-button type="danger" size="medium" @click="closeDialogByButton('newEvaluateSample')">
      ...
      <el-button
        ...
        mini
        @click="submitForm('newEvaluateSample')"
      >
...
export default {
  ...
  methods: {
    ...
    closeDialog() {
      ...
      setTimeout(() => {
        this.$refs["newEvaluateSample"].resetFields();
      }, 300);
    },
    ...
    submitForm(fromData) {
      this.$refs[fromData].validate((valid) => {
        if (valid) {
          ...
          SystemConfigService.createEvaluateSample(this.newEvaluateSample)
            .then((response) => {
