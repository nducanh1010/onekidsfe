...
export default {
  ...
  methods: {
    seachSalaryDefault(id) {
      this.idSalaryDefault = id;
      EmployeeSalaryService.seachSalaryDefault(this.idSalaryDefault)
        .then((resp) => {
        ...
        .catch((err) => {
          ...
          this.$parent.searchEmloyeeSalaryDefault(this.idSalaryDefault);
        });
    },
    closeDialog() {
    ...
    submitForm() {
      this.dataInput.id = this.idSalaryDefault;
      this.$refs["dataInput"].validate((valid) => {
        if (valid) {
          this.checkBeforeSubmit();
          this.loadingButton = true;
          EmployeeSalaryService.updateEmployeeSalaryDefault(this.dataInput)
            .then((resp) => {
              this.$message({
