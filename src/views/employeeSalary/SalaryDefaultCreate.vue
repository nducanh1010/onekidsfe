<template>
  <el-dialog
    title="Thêm khoản công lương mặc định"
  ...
  >
    ...
    >
      <el-row :gutter="20">
        <el-col :span="16">
          <el-form-item label="Tên khoản" prop="value">
...
export default {
  ...
  methods: {
    closeDialog() {
      this.value = "";
      this.$emit("dialog-close", this.idInfo);
      setTimeout(() => {
    ...
    submitForm() {
      this.$refs["dataInput"].validate((valid) => {
        if (valid) {
          ...
          EmployeeSalaryService.createEmployeeSalaryDefault(this.idInfo, this.dataInput)
            .then((resp) => {
              this.$message({
            ...
            .finally(() => {
              ...
              this.$parent.searchEmloyeeSalaryDefault(this.idInfo);
            });
        } else {
    ...
    getSalaryDefaultInitial() {
      this.dataInput.category = "out";
    },
  },
};
</script>
